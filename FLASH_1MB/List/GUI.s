///////////////////////////////////////////////////////////////////////////////
//                                                                            /
// IAR ANSI C/C++ Compiler V6.60.1.5097/W32 for ARM     26/Nov/2014  20:35:20 /
...
//                    \ShunAn_Panel\GUI.c -D IAR -D TWR_K60F120M -lCN         /
...
// D:\´´×÷ÊÒ\·ÉË¼¿¨¶ûÖÇÄÜÐ¡³µ\Ë³°µÈýºÅ\´úÂë\MK60FX512VLQ15\ShunAn_Panel\GUI.c
//    1 #include "arm_cm4.h"
//    2 #include "MK60F15.h"
...
//  124 /******************************************************************************/
//  125 /******************************************************************************/
//  126 // ×ø±êÏµ¿Ø¼þ£¨Ò»ÌõÏß£©

        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  127 uint8 (_v_Axis_Create)( void * target_axis_Input ) // ²ÎÊýÐÞ¸Äº¯Êý
//  128 {
_v_Axis_Create:
        PUSH     {R4-R7,LR}
        SUB      SP,SP,#+12
//  129     _s_GUI_Axis * target_axis = (_s_GUI_Axis *)(((s_Control*)target_axis_Input)->Name);
        LDR      R4,[R0, #+8]
//  130     // »æÖÆ¼ýÍ·
//  131     if(0 == target_axis->Position.angle) // ½Ç¶ÈÎªÁã£¬ÒÔx·½ÏòÎªÕý·½Ïò
        LDRH     R0,[R4, #+4]
        CMP      R0,#+0
        BNE.N    ??_v_Axis_Create_0
//  132     {
//  133         LCD_SetPath(4); // Éè¶¨·½Ïò
        MOVS     R0,#+4
        BL       LCD_SetPath
...
??_v_Axis_Create_1:
        ...
        BL       LCD_SetLine
//  189     // »æÖÆ×ø±ê¸ñÓëÊý×Ö
//  190     if(0 != target_axis->Scale.minScale) // µ±×îÐ¡Á¿³ÌÎª0Ê±²»»æÖÆ
        LDR      R0,[R4, #+24]
        CMP      R0,#+0
        BEQ.W    ??_v_Axis_Create_2
//  191     {
//  192         float Step_K = ((float)(target_axis->Scale.max - target_axis->Scale.zero) // ËõÐ¡ÏµÊýK
...
//  196                              / target_axis->Scale.minScale; // ¼ÆËã×îÐ¡Á¿³ÌÊýÁ¿
        LDR      R0,[R4, #+20]
        LDR      R1,[R4, #+16]
        SUBS     R0,R0,R1
        LDR      R1,[R4, #+24]
...
//  203         {
//  204             switch(target_axis->Position.angle) // Ñ¡¶¨ÆðÊ¼·½Î»
//  205             {
//  206                 case 0:LCD_SetPath(0); // ÓÒ
...
//  349 }
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  350 uint8(CallBack_Back)( s_Control ** guiGaia ) // ±ê×¼CallBackº¯Êý
//  351 {
CallBack_Back:
        PUSH     {R4,LR}
        MOVS     R4,R0
...
timer:
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  359 uint8(CallBack_Show)(s_Control ** guiGaia) // ±ê×¼CallBackº¯Êý
//  360 {
CallBack_Show:
        PUSH     {R4,R5,LR}
        SUB      SP,SP,#+12
...
//  376 // Data Scope CallBack
        ...
        SECTION `.text`:CODE:NOROOT(1)
        THUMB
//  377 uint8(DataScope_Create)(s_Control ** Topest) // ±ê×¼CallBackº¯Êý
//  378 {
DataScope_Create:
        PUSH     {R4,LR}
        MOVS     R4,R0
